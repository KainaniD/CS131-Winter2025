...
port_to_talks_to = {
    ...
    campbellPort: [bonaPort, baileyPort, jaquezPort], # Campbell -> Bona / Bailey / Jaquez
    clarkPort: [bonaPort, jaquezPort], # Clark -> Bona / Jaquez
...
async def flood_update(message, server_name):
    ...
    propagate_message = "UPDATE " + " ".join(message) + '\n'
    #server name is the same of the current server

    # print("PROPOGATING MESSAGE:", propagate_message)
    ...
    for neighbor_port in port_to_talks_to[name_to_port[server_name]]:
        if not str(neighbor_port) in sent_from:
            try:
                ...
                _, propagate_writer = await asyncio.open_connection('127.0.0.1', neighbor_port)
                propagate_writer.write(propagate_message.encode())
                propagate_writer.close()
                await propagate_writer.wait_closed()
            except:
